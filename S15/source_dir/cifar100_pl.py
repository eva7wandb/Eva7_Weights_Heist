...
class CIFAR100DataModule(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        return DataLoader(self.cifar_test, batch_size=4, num_workers=4)
...
# The pretrained model is from an Imagenet training and hence the model need to be changed
# to suit the number of output classes in CIFAR100 dataset, i.e. we need to change the output
# layer features from 1000 classes to 100 classes. This is done using create_model function
# below by adding a linear layer with output nodes set to 100.
...
def _train(args):
    ...
    '''
    - During current training, checkpoint.pth will be saved inside '/opt/ml/checkpoints'
    - First time the notebook is run, we may not have any checkpoints saved in there. So the control goes to the else section.
    - Download the model if available from gdrive and load it  
    '''
    ...
    if os.path.isfile(args.checkpoint_path + '/checkpoint.pth'):
        ...
        gdd.download_file_from_google_drive(file_id='1AaIgeqoBmo-6BalavWJxbYL2RJN3jS5f', dest_path='./Resnet34_pl_cifar100.pt', unzip=False) 
